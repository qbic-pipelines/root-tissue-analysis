...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/rts/usage](https://nf-co.re/rts/usage)
...
```console
nextflow run nf-core/rts --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/rts releases page](https://github.com/nf-core/rts/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
Several generic profiles are bundled with the pipeline which instruct the pipeline to use software packaged using different methods (Docker, Singularity, Podman, Shifter, Charliecloud, Conda) - see below. When using Biocontainers, most of these software packaging methods pull Docker containers from quay.io e.g [RootSeg](https://quay.io/repository/biocontainers/rootseg) except for Singularity which directly downloads Singularity images via https hosted by the [Galaxy project](https://depot.galaxyproject.org/singularity/) and Conda which downloads and installs software locally from [Bioconda](https://bioconda.github.io/).
